...
class EnsembleForecaster:
    ...
    def predict(self, features: FeatureVector, historical_data: List[MarketData]) -> ForecastOutput:
        ...
        # 6. Ensemble weighting based on regime and confidence
        regime_weights = self._get_regime_weights(regime['regime'])
        
        # Adjust weights based on confidence level
        confidence_adjustment = min(1.0, max(0.5, confidence * 2))  # Scale between 0.5-1.0
        ...
        # Use direction prediction to adjust sign if confidence is high enough
        direction_signal = 0
        if confidence > 0.6:  # Only use direction if confident
            direction_signal = regime_weights['direction'] * (0.005 if direction == 1 else -0.005)
        
        final_return = weighted_xgboost + weighted_lstm + direction_signal
    ...
    def _calculate_risk_metrics(self, quantiles: Dict[str, float], volatility: float) -> Dict[str, float]:
        ...
        max_drawdown = -abs(volatility * 1.5) if volatility > 0 else -0.05  # Typically 1.5x volatility
